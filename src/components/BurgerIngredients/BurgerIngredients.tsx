...
const BurgerIngredients: FC = () => {
  ...
  function handleButtonClick(tab: "buns" | "sauces" | "main") {
    setCurrent(tab);
  ...
  return (
    <section className={`${styles.burgerIngredients} pt-10'`}>
      ...
      <div className={`${styles.tabBlock} pb-10`}>
        <Tab value="buns" active={current === 'buns'} onClick={() => handleButtonClick("buns")}>
          Булки
        </Tab>
        <Tab value="sauces" active={current === 'sauces'} onClick={() => handleButtonClick("sauces")}>
          Соусы
        </Tab>
        <Tab value="main" active={current === 'main'} onClick={() => handleButtonClick("main")}>
          Начинки
        </Tab>
      </div>
